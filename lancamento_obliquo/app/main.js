...
cannon.position.set(5, app.renderer.screen.height - 60);
...
const restartButton = features.addChild(
  new Graphics()
      .beginFill(0xFF0000)
      .lineStyle(4, 0xDF0101, 1)
      .drawRoundedRect(0, 0, 200, 50, 3)
...
restartButton.position.set(49, 250);
...
imagemPerdeuBG.y = 0
  ...
  // Text
buttonTextIniciar = new PIXI.Text("Iniciar!", {
  fontSize: 20, 
  fill: "0xFFFFFF",
  align: 'center', 
  fontFamily: 'Arial',
...
buttonTextIniciar.anchor.set(0.5, 0.5);
buttonTextIniciar.position.set(100, 25);
startButton.addChild(buttonTextIniciar);

buttonTextReiniciar = new PIXI.Text("Reiniciar", {
  fontSize: 20, 
  fill: "0xFFFFFF",
  align: 'center', 
  fontFamily: 'Arial',
  stroke: "0xFFFFFF",
  strokeThickness: 0.01});
buttonTextReiniciar.anchor.set(0.5, 0.5);
buttonTextReiniciar.position.set(100, 25);
restartButton.addChild(buttonTextReiniciar);
...
mouseInfoText = new PIXI.Text('Coords. do Cursor: x = 0.00 m, y = 0.00 m', {
  fontSize: 14, 
  fill: "0x00000",
  align: 'center', 
  fontFamily: 'Arial',
});
mouseInfoText.anchor.set(0, 0);
mouseInfoText.position.set(20, 140);
features.addChild(mouseInfoText);

app.renderer.plugins.interaction.on('mousemove', onMove);

function onMove (event) {
  if (event.data.global.x >= 0 && event.data.global.y >= 0 && event.data.global.x <=700 & event.data.global.y <= 400) {
    mouseInfoText.text = `Coords. do Cursor: x = ${Math.round(event.data.global.x) / 10} m, y = ${Math.round(350 - event.data.global.y) / 10} m`;
  }
}
...
startButton.addEventListener('pointerdown', (anEvent) => {
  ...
  app.ticker.add((delta) => {
      ...
      }else if(ballPhysics.position[1] >= 335 && ballPhysics.position[0] >= 30 && ballPhysics._xVelocity < 0){
        //app.ticker.destroy();
...
restartButton.addEventListener('pointerdown', (anEvent) => {
  mainStage.removeChild(imagemPerdeuBG)
  mainStage.removeChild(imagemGanharBG)
  startButton.interactive = true;
  theta = 10
  thetaHandle.position.set(37.5, 25 + 45 + 5 / 2);
  thetaInfoText.text = `Ângulo inicial: θ = ${theta.toFixed(1)}°`;
